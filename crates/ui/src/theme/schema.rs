...
impl ThemeColor {
    ...
    pub(crate) fn apply_config(&mut self, config: &ThemeConfig, default_theme: &ThemeColor) {
        ...
        self.list_active.alpha = self.list_active.alpha.min(0.2);
        self.table_active.alpha = self.table_active.alpha.min(0.2);
        self.selection.alpha = self.selection.alpha.min(0.3);
    }
}
