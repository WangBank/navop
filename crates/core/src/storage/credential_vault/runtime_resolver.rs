...
impl ConnectionRepository {
    ...
    ///
    /// 与数据库/中间件走同一凭据解析机制:`config[field].credential_reference`
    /// 被替换为 `config[field] = {"username": ...}`,密码合并进 secrets
    /// (`<field>.password`),沿用 secret://self 由 provider 运行时拉取。仅用于
    /// 内存中的连接尝试,禁止落盘/同步/日志。
    fn resolve_extension_tunnel(
        ...
        resolve_extension_auth_objects(
            &mut config,
        ...
        let mut secrets = params.secrets;
        secrets.extend(resolved_secrets);
...
/// `config[field] = {"username": ...}`,密码写入 `secret_fields[<field>.password]`。
/// `fetch` 由调用方提供真实密码簿查找,便于纯函数测试。
fn resolve_extension_auth_objects(
    config: &mut Map<String, Value>,
    secret_fields: &mut BTreeMap<String, String>,
    ...
    for (field, value) in config.iter() {
        ...
        if reference.password {
            if let Some(password) = password.filter(|value| !value.is_empty()) {
                secret_fields.insert(format!("{field}.password"), password);
            }
        }
...
mod tests {
    ...
    #[test]
    fn resolves_reference_into_username_and_secret_password() {
        ...
        assert_eq!(Some(&"s3cret".to_string()), secrets.get("auth.password"));
    }
    ...
    fn leaves_non_auth_and_manual_fields_untouched() {
        ...
        let mut secrets = BTreeMap::new();
        resolve_extension_auth_objects(&mut config, &mut secrets, |_| unreachable!()).unwrap();
    ...
    fn honors_reference_field_flags() {
        ...
        let mut secrets = BTreeMap::new();
        resolve_extension_auth_objects(&mut config, &mut secrets, |_| {
            Ok((Some("user".into()), Some("pw".into())))
        })
        .unwrap();
        assert_eq!(config["auth"]["username"], "user");
        assert!(secrets.is_empty(), "password 标志关闭时不产出 secret");
    }
    ...
    fn propagates_missing_credential_error() {
        ...
        let mut secrets = BTreeMap::new();
        let error = resolve_extension_auth_objects(&mut config, &mut secrets, |_| {
